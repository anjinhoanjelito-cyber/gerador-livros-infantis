...
async function generateBook(formData) {
    try {
        ...
        updateProgress(100, 'Completo', 4, true);
...
async function callGeminiAPI(formData) {
    logDebug('Chamando Gemini DIRETO');
...
{"title":"Titulo","pages":[{"pageNumber":1,"text":"Texto da pagina","needsIllustration":true,"illustrationPrompt":"descricao em ingles"}],"mainCharacters":[{"name":"Nome","visualReference":"descricao"}],"colorPalette":["#hex"],"mood":"alegre","setting":"cenario"}`;
    ...
    if (!data.candidates || !data.candidates[0]) {
        logDebug('Resposta: ' + JSON.stringify(data));
        throw new Error('Resposta invalida do Gemini');
    }

    const text = data.candidates[0].content.parts[0].text;
    logDebug('Texto: ' + text.substring(0, 100));
...
async function generateCoverImage(story) {
    return await generateImage(`Children's book cover: ${story.title}, vibrant, colorful, professional, NO TEXT`);
}
...
async function generateImage(prompt) {
    ...
    logDebug('Prediction ID: ' + result.id);
    ...
    while (result.status !== 'succeeded' && result.status !== 'failed' && attempts < 60) {
        ...
        attempts++;
        if (attempts % 10 === 0) logDebug('Aguardando... ' + attempts);
    }

    if (result.status === 'failed') throw new Error('Imagem falhou');
    if (!result.output || !result.output[0]) throw new Error('Sem output');
